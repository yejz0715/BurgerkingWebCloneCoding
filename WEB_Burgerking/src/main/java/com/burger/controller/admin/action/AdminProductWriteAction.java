...
public class AdminProductWriteAction implements Action {
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * (AdminVO)session.getAttribute("loginAdmin"); if(avo == null) { url =
		 * "shop.do?command=admin"; }else { // 현재 프로젝트의 세션을 통해 적용할 저장경로 얻음
		 * ServletContext context = session.getServletContext(); String uploadFilePath =
		 * context.getRealPath("product_images");
		 * 
		 * // MultipartRequest 객체 생성 - 파일 업로드 MultipartRequest multi = new
		 * MultipartRequest( request, uploadFilePath, 5*1024*1024, "UTF-8", new
		 * DefaultFileRenamePolicy());
		 * 
		 * // 나머지 파라미터들을 dto에 저장 ProductVO pvo = new ProductVO();
		 ...
		 * 
		 * AdminDao adao = AdminDao.getInstance(); adao.insertProduct(pvo); }
