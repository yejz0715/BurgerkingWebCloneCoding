...
public class AdminProductUpdateAction implements Action {
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * if(avo == null) url = "shop.do?command=admin"; else { ProductVO pvo = new
		 * ProductVO();
		 * 
		 ...
		 * pvo.setImage(multi.getParameter("oldImage")); else
		 * pvo.setImage(multi.getFilesystemName("image"));
		 * 
		 * AdminDao adao = AdminDao.getInstance(); adao.updateProduct(pvo);
		 * 
		 * url = url + "&pseq=" + pvo.getPseq(); } response.sendRedirect(url);
		 */
	}
}
