...
public class OrderDao {
	...
	public int nonInsertOrder(ArrayList<CartVO> list, String id) {
		int oseq = 0; // orders 테이블에 추가된 주문번호를 리턴할 에정입니다
		con = DBman.getConnection();
	...
	public ArrayList<orderVO> nonListOrderById(String id, int oseq) {
		ArrayList<orderVO> list = new ArrayList<orderVO>();
		String sql = "select * from non_order_view  where id=? and result ='1' and oseq=?";
		con = DBman.getConnection();
		try {
			  pstmt = con.prepareStatement(sql); 
			  pstmt.setString(1, id);
